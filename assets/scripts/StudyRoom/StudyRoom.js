...
cc.Class({
    ...
    renderUpBox(type){
        ...
        if(type==1){//种植技术
           ...
           /* this.getPlantTec().then((res)=>{
            ...
            let desc = "邀请好友1人可升级2级林权证，可开垦第7块地;\n" +
                        "邀请好友3人可升级3级林权证，可开垦第8块地;\n"+
                        "邀请好友5人可升级4级林权证，可开垦第9块地;\n"+
                        "邀请好友10人可升级5级林权证，可开垦第10块地;\n"+
                        "邀请好友20人可升级6级林权证，可开垦第11块地;\n"+
                        "邀请好友50人可升级7级林权证，可开垦第12块地.";

            this.boxDesc.string = desc;
            this.oprBtnBox.active = false;
            let lv = cc.director.getScene().getChildByName('PersistNode').getComponent('PersistNode').userData.selfInfo.scientific.certLev||0;
            this.lv.string = "<color=#ffd07e><outline color=#562B04 width=2>当前等级 : "+lv+"</outline></color>";
            // 林权证后台数据
    ...
    getWoodCer(){
        var promise = new Promise((resolve,reject)=>{
            Net.get('/game/scientific/cert',1,null,(data)=>{
                if(!data.success){
    ...
    getWoodMarket(){
        var promise = new Promise((resolve,reject)=>{
            Net.get('/game/scientific/woodca',1,null,(data)=>{
                if(!data.success){
                    this.showLittleTip(data.msg);
                    return;
                }else{
    ...
    getTanMarket(){
        var promise = new Promise((resolve,reject)=>{
            Net.get('/game/scientific/greenca',1,null,(data)=>{
                if(!data.success){
                    this.showLittleTip(data.msg);
                    return;
                }else{
    ...
    upPlantTec(){
        return new Promise((resolve,reject)=>{
            Net.get('/game/scientific/plantUp',1,null,(res)=>{
                if(!res.success){
    ...
    upWoodCer(){
        return new Promise((resolve,reject)=>{
            Net.get('/game/scientific/certUp',1,null,(res)=>{
                if(!res.success){
                    this.showLittleTip(res.msg);
                }else{
                    resolve(res.obj);
    ...
    activeWoodMar(){
        return new Promise((resolve,reject)=>{
            Net.get('/game/scientific/woodActivate',1,null,(res)=>{
                if(!res.success){
                    this.showLittleTip(res.msg);
                }else{
                    resolve(res.obj);
    ...
    activeTanMar(){
        return new Promise((resolve,reject)=>{
            Net.get('/game/scientific/greenActivate',1,null,(res)=>{
                if(!res.success){
                    this.showLittleTip(res.msg);
                }else{
                    resolve(res.obj);
    ...
    renderNeedMerBox(arr){
        ...
        //得到仓库中所有木材
        Net.get('/game/getPlayerItemList',1,{type:2},(res)=>{
            if(!res.success){
                this.showLittleTip(res.msg);
            }else{
                var wooList = res.obj;
