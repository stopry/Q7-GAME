...
cc.Class({
    ...
    toMarket(){
        ...
        Util.toAppMarket('market',this.id);
        /*
        let token = cc.sys.localStorage.getItem('token');
        token = encodeURI(token);
        let id = this.id;
        cc.sys.openURL("http://wap.market.o2plan.cn/html/skip-page.html?id="+id+"&token="+token+"&link=market");
        */
        //cc.sys.openURL("http://localhost/html/skip-page.html?id="+id+"&token="+token+"&link=market");
