...
cc.Class({
    ...
    selMoney(){//选择金额框
        ...
        ;[].forEach.call(self.item,function(item){
            item.on(cc.Node.EventType.TOUCH_END,function(event){
                ...
                this.item[3].getComponent(cc.Sprite).spriteFrame = this.bgPic[0];
    ...
    showConDia(){//弹出确认对话框
        ...
        dia.getComponent('ConfirmDia').setBoxFun('确定要用'+this.needMoney+'金币兑换'+this.exchangeJewel+'钻石吗？',function(){
            //cc.log('ok');
            this.confirmExh();
        }.bind(this),function(){
            //cc.log('no');
    ...
    //去交易界面充值
    toRecharge(){
        util.toAppMarket('gold-recharge');
        /*
        let token = cc.sys.localStorage.getItem('token');
        token = encodeURI(token);
        cc.sys.openURL(Global.marketDomain+"/html/skip-page.html?token="+token+"&link=gold-recharge");
        */
        // cc.sys.openURL("http://localhost:4200/#/skipPage?token="+token+"&link=goldRecharge");
    },
    confirmExh(){//确认兑换钻石
        var self = this;
        Net.get('/game/jewel/convert',1,{num:self.exchangeJewel},function(data){
            if(!data.success){
    ...
    //刷新可用金币
    freshMymoney(){
        Net.get('/game/jewel/list',1,null,function(data){
            if(!data.success){
                this.showLittleTip(data.msg);
                return;
            }
            this.myMoney.string = (data.obj.money).toFixed(2);
        }.bind(this), function (err) {
